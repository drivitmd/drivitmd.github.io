...
(function(){var ORD={early:0,mid:1,late:2};var st=(new Date()<=EARLY)?'early':(new Date()<=MID)?'mid':'late';document.querySelectorAll('.tier').forEach(function(t){var d=t.dataset.tier;t.classList.toggle('active',d===st);t.classList.toggle('done',ORD[d]<ORD[st])});var sp=document.getElementById('stickyPrice');if(sp)sp.textContent=st==='early'?'250 €':st==='mid'?'300 €':'—'})();
